...
public class LoadEssentialWarpModule {
    public LoadEssentialWarpModule(FallCraftSystem fallCraftSystem) {
        try {
            ...
            fallCraftSystem.getServer().getConsoleSender().sendMessage(MethodsStatics.formater(PluginInfo.PLUGIN_NAME + "&cModule &f>> &aWarp load"));
        } catch (Exception e) {
            fallCraftSystem.getServer().getConsoleSender().sendMessage(MethodsStatics.formater(PluginInfo.PLUGIN_NAME + "&cModule &f>> &cWarp not load"));
        }
