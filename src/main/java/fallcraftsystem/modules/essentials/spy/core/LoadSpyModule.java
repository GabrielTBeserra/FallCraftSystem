...
public class LoadSpyModule {
    public LoadSpyModule(FallCraftSystem plugin) {
        ...
        new SpyCommand(plugin);
            plugin.getServer().getConsoleSender().sendMessage(MethodsStatics.formater(PluginInfo.PLUGIN_NAME + "&cModule &f>> &aSpy load"));
    } catch (Exception e) {
            plugin.getServer().getConsoleSender().sendMessage(MethodsStatics.formater(PluginInfo.PLUGIN_NAME + "&cModule &f>> &cSpy not load"));
    }
    }
}
